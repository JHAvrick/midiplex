...
/**
   ...
   * @extends EventEmitter
   * @license Apache-2.0
   */const wm=new class extends EventEmitter{constructor(){super(),this.defaults={note:{attack:Utilities.from7bitToFloat(64),release:Utilities.from7bitToFloat(64),duration:1/0}},this.interface=null,this.validation=!0,this._inputs=[],this._disconnectedInputs=[],this._outputs=[],this._disconnectedOutputs=[],this._stateChangeQueue=[],this._octaveOffset=0}async enable(options={},legacy=!1){if(Utilities.isNode){try{window.navigator}catch(err){global.navigator=await Object.getPrototypeOf((async function(){})).constructor('\n        let jzz = await import("jzz");\n        return jzz.default;\n        ')()}try{performance}catch(err){global.performance=await Object.getPrototypeOf((async function(){})).constructor('\n        let perf_hooks = await import("perf_hooks");\n        return perf_hooks.performance;\n        ')()}}if(this.validation=!1!==options.validation,this.validation&&("function"==typeof options&&(options={callback:options,sysex:legacy}),legacy&&(options.sysex=!0)),this.enabled)return"function"==typeof options.callback&&options.callback(),Promise.resolve();const errorEvent={timestamp:this.time,target:this,type:"error",error:void 0},midiAccessGrantedEvent={timestamp:this.time,target:this,type:"midiaccessgranted"},enabledEvent={timestamp:this.time,target:this,type:"enabled"};try{"function"==typeof options.requestMIDIAccessFunction?this.interface=await options.requestMIDIAccessFunction({sysex:options.sysex,software:options.software}):this.interface=await navigator.requestMIDIAccess({sysex:options.sysex,software:options.software})}catch(err){return errorEvent.error=err,this.emit("error",errorEvent),"function"==typeof options.callback&&options.callback(err),Promise.reject(err)}this.emit("midiaccessgranted",midiAccessGrantedEvent),this.interface.onstatechange=this._onInterfaceStateChange.bind(this);try{await this._updateInputsAndOutputs()}catch(err){return errorEvent.error=err,this.emit("error",errorEvent),"function"==typeof options.callback&&options.callback(err),Promise.reject(err)}return this.emit("enabled",enabledEvent),"function"==typeof options.callback&&options.callback(),Promise.resolve(this)}async disable(){return this.interface&&(this.interface.onstatechange=void 0),this._destroyInputsAndOutputs().then((()=>{navigator&&"function"==typeof navigator.close&&navigator.close(),this.interface=null;let event={timestamp:this.time,target:this,type:"disabled"};this.emit("disabled",event),this.removeListener()}))}getInputById(id,options={disconnected:!1}){if(this.validation){if(!this.enabled)throw new Error("WebMidi is not enabled.");if(!id)return}if(options.disconnected){for(let i=0;i<this._disconnectedInputs.length;i++)if(this._disconnectedInputs[i].id===id.toString())return this._disconnectedInputs[i]}else for(let i=0;i<this.inputs.length;i++)if(this.inputs[i].id===id.toString())return this.inputs[i]}getInputByName(name,options={disconnected:!1}){if(this.validation){if(!this.enabled)throw new Error("WebMidi is not enabled.");if(!name)return;name=name.toString()}if(options.disconnected){for(let i=0;i<this._disconnectedInputs.length;i++)if(~this._disconnectedInputs[i].name.indexOf(name))return this._disconnectedInputs[i]}else for(let i=0;i<this.inputs.length;i++)if(~this.inputs[i].name.indexOf(name))return this.inputs[i]}getOutputByName(name,options={disconnected:!1}){if(this.validation){if(!this.enabled)throw new Error("WebMidi is not enabled.");if(!name)return;name=name.toString()}if(options.disconnected){for(let i=0;i<this._disconnectedOutputs.length;i++)if(~this._disconnectedOutputs[i].name.indexOf(name))return this._disconnectedOutputs[i]}else for(let i=0;i<this.outputs.length;i++)if(~this.outputs[i].name.indexOf(name))return this.outputs[i]}getOutputById(id,options={disconnected:!1}){if(this.validation){if(!this.enabled)throw new Error("WebMidi is not enabled.");if(!id)return}if(options.disconnected){for(let i=0;i<this._disconnectedOutputs.length;i++)if(this._disconnectedOutputs[i].id===id.toString())return this._disconnectedOutputs[i]}else for(let i=0;i<this.outputs.length;i++)if(this.outputs[i].id===id.toString())return this.outputs[i]}noteNameToNumber(name){return this.validation&&console.warn("The noteNameToNumber() method is deprecated. Use Utilities.toNoteNumber() instead."),Utilities.toNoteNumber(name,this.octaveOffset)}getOctave(number){return this.validation&&(console.warn("The getOctave()is deprecated. Use Utilities.getNoteDetails() instead"),number=parseInt(number)),!isNaN(number)&&number>=0&&number<=127&&Utilities.getNoteDetails(Utilities.offsetNumber(number,this.octaveOffset)).octave}sanitizeChannels(channel){return this.validation&&console.warn("The sanitizeChannels() method has been moved to the utilities class."),Utilities.sanitizeChannels(channel)}toMIDIChannels(channel){return this.validation&&console.warn("The toMIDIChannels() method has been deprecated. Use Utilities.sanitizeChannels() instead."),Utilities.sanitizeChannels(channel)}guessNoteNumber(input){return this.validation&&console.warn("The guessNoteNumber() method has been deprecated. Use Utilities.guessNoteNumber() instead."),Utilities.guessNoteNumber(input,this.octaveOffset)}getValidNoteArray(notes,options={}){return this.validation&&console.warn("The getValidNoteArray() method has been moved to the Utilities.buildNoteArray()"),Utilities.buildNoteArray(notes,options)}convertToTimestamp(time){return this.validation&&console.warn("The convertToTimestamp() method has been moved to Utilities.toTimestamp()."),Utilities.toTimestamp(time)}async _destroyInputsAndOutputs(){let promises=[];return this.inputs.forEach((input=>promises.push(input.destroy()))),this.outputs.forEach((output=>promises.push(output.destroy()))),Promise.all(promises).then((()=>{this._inputs=[],this._outputs=[]}))}_onInterfaceStateChange(e){this._updateInputsAndOutputs();let event={timestamp:e.timeStamp,type:e.port.state,target:this};if("connected"===e.port.state&&"open"===e.port.connection){"output"===e.port.type?event.port=this.getOutputById(e.port.id):"input"===e.port.type&&(event.port=this.getInputById(e.port.id)),this.emit(e.port.state,event);const portsChangedEvent=Object.assign({},event);portsChangedEvent.type="portschanged",this.emit(portsChangedEvent.type,portsChangedEvent)}else if("disconnected"===e.port.state&&"pending"===e.port.connection){"input"===e.port.type?event.port=this.getInputById(e.port.id,{disconnected:!0}):"output"===e.port.type&&(event.port=this.getOutputById(e.port.id,{disconnected:!0})),this.emit(e.port.state,event);const portsChangedEvent=Object.assign({},event);portsChangedEvent.type="portschanged",this.emit(portsChangedEvent.type,portsChangedEvent)}}async _updateInputsAndOutputs(){return Promise.all([this._updateInputs(),this._updateOutputs()])}async _updateInputs(){if(!this.interface)return;for(let i=this._inputs.length-1;i>=0;i--){const current=this._inputs[i];Array.from(this.interface.inputs.values()).find((input=>input===current._midiInput))||(this._disconnectedInputs.push(current),this._inputs.splice(i,1))}let promises=[];return this.interface.inputs.forEach((nInput=>{if(!this._inputs.find((input=>input._midiInput===nInput))){let input=this._disconnectedInputs.find((input=>input._midiInput===nInput));input||(input=new Input(nInput)),this._inputs.push(input),promises.push(input.open())}})),Promise.all(promises)}async _updateOutputs(){if(!this.interface)return;for(let i=this._outputs.length-1;i>=0;i--){const current=this._outputs[i];Array.from(this.interface.outputs.values()).find((output=>output===current._midiOutput))||(this._disconnectedOutputs.push(current),this._outputs.splice(i,1))}let promises=[];return this.interface.outputs.forEach((nOutput=>{if(!this._outputs.find((output=>output._midiOutput===nOutput))){let output=this._disconnectedOutputs.find((output=>output._midiOutput===nOutput));output||(output=new Output(nOutput)),this._outputs.push(output),promises.push(output.open())}})),Promise.all(promises)}get enabled(){return null!==this.interface}get inputs(){return this._inputs}get isNode(){return this.validation&&console.warn("WebMidi.isNode has been deprecated. Use Utilities.isNode instead."),Utilities.isNode}get isBrowser(){return this.validation&&console.warn("WebMidi.isBrowser has been deprecated. Use Utilities.isBrowser instead."),Utilities.isBrowser}get octaveOffset(){return this._octaveOffset}set octaveOffset(value){if(this.validation&&(value=parseInt(value),isNaN(value)))throw new TypeError("The 'octaveOffset' property must be an integer.");this._octaveOffset=value}get outputs(){return this._outputs}get supported(){return"undefined"!=typeof navigator&&navigator.requestMIDIAccess}get sysexEnabled(){return!(!this.interface||!this.interface.sysexEnabled)}get time(){return performance.now()}get version(){return"3.1.6"}get flavour(){return"esm"}get CHANNEL_EVENTS(){return this.validation&&console.warn("The CHANNEL_EVENTS enum has been moved to Enumerations.CHANNEL_EVENTS."),Enumerations.CHANNEL_EVENTS}get MIDI_SYSTEM_MESSAGES(){return this.validation&&console.warn("The MIDI_SYSTEM_MESSAGES enum has been moved to Enumerations.SYSTEM_MESSAGES."),Enumerations.SYSTEM_MESSAGES}get MIDI_CHANNEL_MODE_MESSAGES(){return this.validation&&console.warn("The MIDI_CHANNEL_MODE_MESSAGES enum has been moved to Enumerations.CHANNEL_MODE_MESSAGES."),Enumerations.CHANNEL_MODE_MESSAGES}get MIDI_CONTROL_CHANGE_MESSAGES(){return this.validation&&console.warn("The MIDI_CONTROL_CHANGE_MESSAGES enum has been replaced by the Enumerations.CONTROL_CHANGE_MESSAGES array."),Enumerations.MIDI_CONTROL_CHANGE_MESSAGES}get MIDI_REGISTERED_PARAMETER(){return this.validation&&console.warn("The MIDI_REGISTERED_PARAMETER enum has been moved to Enumerations.REGISTERED_PARAMETERS."),Enumerations.REGISTERED_PARAMETERS}get NOTES(){return this.validation&&console.warn("The NOTES enum has been deprecated."),["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"]}};wm.constructor=null;class MidiplexNodeInstance{constructor(key,node,config={}){var _a,_b,_c,_d;this.props=new Map,this.state=new Map,this.inputs=new Map,this.outputs=new Map,this.receiveHandler=null,this.updateHandler=null,this.key=key,this.definition=node;for(let key in node.props){let k=key;this.props.set(k,null!==(_b=null===(_a=config.props)||void 0===_a?void 0:_a[k])&&void 0!==_b?_b:node.props[k].value)}for(let key in node.state){let k=key;this.state.set(k,node.state[k].value)}for(let key in node.inputs)this.inputs.set(key,{key:key,messageTypes:node.inputs[key].messageTypes,node:this,to:[]});for(let key in node.outputs)this.outputs.set(key,{key:key,messageTypes:node.outputs[key].messageTypes,node:this,to:[]});this.getProp=this.getProp.bind(this),this.setProp=this.setProp.bind(this),this.getState=this.getState.bind(this),this.getOrSetStateInternal=this.getOrSetStateInternal.bind(this),null===(_d=(_c=this.definition).node)||void 0===_d||_d.call(_c,{prop:this.getProp.bind(this),state:this.getOrSetStateInternal.bind(this),send:this.send.bind(this),receive:this.bindReceive.bind(this),update:this.bindUpdate.bind(this)})}bindReceive(handler){if(this.receiveHandler)throw new Error(`Receive handler already bound to node ${this.key}.`);this.receiveHandler=handler}bindUpdate(handler){if(this.updateHandler)throw new Error(`Update handler already bound to node ${this.key}.`);this.updateHandler=handler}getOrSetStateInternal(stateKey,value){return void 0!==value&&this.state.set(stateKey,value),this.state.get(stateKey)}getState(stateKey){return this.state.get(stateKey)}getProp(propKey){return this.props.get(propKey)}setProp(propKey,value){var _a;return this.props.set(propKey,value),null===(_a=this.updateHandler)||void 0===_a||_a.call(this),value}getDefaultEdge(){return this.inputs.values().next().value}getInputEdge(edgeKey){let edge=this.inputs.get(edgeKey);if(edge)return edge;throw new Error(`Input edge ${edgeKey} does not exist.`)}connect(edgeKey,to){let edge=this.outputs.get(edgeKey);if(!edge)throw Error(`Output edge ${edge} does not exist.`);if(edge.to.includes(to))throw Error(`Output edge ${edge} is already connected to ${to}.`);edge.to.push(to)}disconnect(edgeKey,to){let edge=this.outputs.get(edgeKey);return!!edge&&(edge.to=edge.to.filter((edge=>edge!==to)),!0)}send(message,edgeKey){let edge=edgeKey?this.outputs.get(edgeKey):this.getDefaultEdge();if(edge){if(edge.messageTypes.includes(message.type))return void edge.to.forEach((receivingEdge=>{receivingEdge.node.receive(message,receivingEdge.key)}));throw Error(`Message type "${message.type}" is not supported by edge ${edgeKey} for node type ${this.definition.key}.`)}throw Error(`Output edge "${edge}" does not exist.`)}receive(message,edge){var _a;let edgeInstance=this.inputs.get(edge);if(edgeInstance){if(edgeInstance.messageTypes.includes(message.type))return void(null===(_a=this.receiveHandler)||void 0===_a||_a.call(this,message,edge));let thruEdge=this.outputs.get("thru");if(thruEdge)return void thruEdge.to.forEach((receivingEdge=>{receivingEdge.node.receive(message,receivingEdge.key)}))}}}const AllMessageTypes=["noteoff","noteon","controlchange","keyaftertouch","programchange","channelaftertouch","pitchbend","allnotesoff","allsoundoff","localcontrol","monomode","omnimode","resetallcontrollers","nrpn","nrpn-dataentrycoarse","nrpn-dataentryfine","nrpn-dataincrement","nrpn-datadecrement","rpn","rpn-dataentrycoarse","rpn-dataentryfine","rpn-dataincrement","rpn-datadecrement"],ChannelMessageTypes=["noteoff","noteon","controlchange","keyaftertouch","programchange","channelaftertouch","pitchbend"],convertRange=(oldVal,oldMin,oldMax,newMin,newMax)=>(oldVal-oldMin)*(newMax-newMin)/(oldMax-oldMin)+newMin,clamp=(num,min,max)=>Math.min(Math.max(num,min),max),notes=["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"],noteToMidi=note=>{const octave=parseInt(note.slice(-1)),key=note.slice(0,-1),semitone=notes.indexOf(key);if(-1===semitone||octave<-1||octave>9)throw new Error("Invalid note format");return 12*octave+semitone+12},isChannelMessage=data=>(240&data[0])>=128&&(240&data[0])<=239,setChannel=(data,channel)=>{if(isChannelMessage(data)){channel=clamp(channel,1,16);let statusByte=240&data[0];data[0]=statusByte+channel-1}return data},Util=Object.freeze({Math:{convertRange:convertRange,clamp:clamp},Data:{isChannelMessage:isChannelMessage,setChannel:setChannel},Message:{create:(data,channel)=>(channel&&setChannel(data,channel),new MidiplexMessage(data)),matchTrigger:(trigger,m)=>{if("test"in trigger)return trigger.test(m);switch(!0){case"noteon"===trigger.type&&"noteon"===m.type:case"noteoff"===trigger.type&&"noteoff"===m.type:return!("note"in trigger)||trigger.note===m.data[1];case"controlchange"===trigger.type&&"controlchange"===m.type:return"cc"in trigger&&"value"in trigger?trigger.cc===m.data[1]&&trigger.value===m.data[2]:"cc"in trigger?trigger.cc===m.data[1]:!("value"in trigger)||trigger.value===m.data[2];default:return!1}}},Note:{noteToMidi:noteToMidi},Generate:{noteon:(note,velocity,channel)=>("string"==typeof note&&(note=noteToMidi(note)),Util.Message.create(new Uint8Array([144,note,velocity]),channel)),noteoff:(note,velocity,channel)=>("string"==typeof note&&(note=noteToMidi(note)),Util.Message.create(new Uint8Array([128,note,velocity]),channel)),controlchange:(cc,value,channel)=>Util.Message.create(new Uint8Array([176,cc,value]),channel),keyaftertouch:(note,pressure,channel)=>("string"==typeof note&&(note=noteToMidi(note)),Util.Message.create(new Uint8Array([160,note,pressure]),channel)),programchange:(program,channel)=>Util.Message.create(new Uint8Array([192,program]),channel),pitchbend:(value,channel)=>Util.Message.create(new Uint8Array([224,value]),channel),channelaftertouch:(pressure,channel)=>Util.Message.create(new Uint8Array([208,pressure]),channel)}});class MidiplexMessage{constructor(message,trace=[]){this.message=message instanceof Message?message:new Message(message),this.trace=trace}addTrace(nodeKey){this.trace.push(nodeKey)}hasTrace(nodeKey){return this.trace.includes(nodeKey)}setChannel(channel){if(channel=clamp(channel,1,16),this.message.isChannelMessage){const MESSAGE_TYPE=240&this.message.data[0];this.message=new Message(new Uint8Array([MESSAGE_TYPE+channel-1,this.message.data[1],this.message.data[2]]))}}clone(channel){let m=new MidiplexMessage(new Uint8Array([...this.message.data]),this.trace);return channel&&m.setChannel(channel),m}get isChannelMessage(){return this.message.isChannelMessage}get channel(){return this.message.channel}get type(){return this.message.type}get data(){return this.message.data}}const InputNodeDefinition={name:"Input",key:"INPUT_NODE",description:"A node that receives MIDI messages from a MIDI device. Messages are immediately sent to the node's output.",inputs:{},outputs:{out:{name:"Out",messageTypes:AllMessageTypes}},props:{inputId:{name:"Input",value:null}},node:({prop:prop,send:send,update:update,receive:receive})=>{let inputId=prop("inputId");const handleInputMessage=event=>{send(new MidiplexMessage(event.message,[]),"out")};update((()=>{(()=>{if(inputId){const input=wm.getInputById(inputId);input&&input.removeListener("midimessage",handleInputMessage)}})(),inputId=prop("inputId"),(()=>{if(inputId){const input=wm.getInputById(inputId);if(input)return void input.addListener("midimessage",handleInputMessage);console.warn(`Input ${inputId} not found`)}})()}))}};const OutputNodeDefinition={name:"Input",key:"INPUT_NODE",description:"A node that receives MIDI messages from a MIDI device. Messages are immediately sent to the node's output.",inputs:{in:{name:"Out",messageTypes:AllMessageTypes}},outputs:{},props:{outputId:{name:"Output",value:null}},node:({prop:prop,send:send,update:update,receive:receive})=>{let output,outputId=prop("outputId");outputId&&(output=wm.getOutputById(outputId),output||console.warn(`Output ${outputId} not found`)),receive(((message,edge)=>{output&&output.send(message.data)})),update((()=>{outputId=prop("outputId"),outputId&&(output=wm.getOutputById(outputId),output||console.warn(`Output ${outputId} not found`))}))}};const DebugNodeDef={name:"Debug",key:"DEBUG_NODE",description:"",inputs:{in:{name:"In",messageTypes:AllMessageTypes}},outputs:{out:{name:"Out",messageTypes:AllMessageTypes}},props:{logToConsole:{name:"Log to console",value:!1},callback:{name:"Callback",value:()=>{}}},node({send:send,receive:receive,prop:prop}){receive(((message,edge)=>{prop("logToConsole")&&console.log(message),prop("callback")(message),send(message,"out")}))}};const MessageTypePassNodeDef={name:"Message Type Filter",key:"MESSAGE_TYPE_FILTER_NODE",description:"",inputs:{in:{name:"In",messageTypes:AllMessageTypes}},outputs:{out:{name:"Out",messageTypes:AllMessageTypes}},props:{messageTypes:{name:"Message Types",value:AllMessageTypes}},node({send:send,receive:receive,prop:prop}){receive((message=>{prop("messageTypes").includes(message.type)&&send(message,"out")}))}};let outputs=Object.fromEntries(AllMessageTypes.map((type=>[type,{name:type,messageTypes:[type]}])));const MessageTypeSplitDef={name:"Message Type Splitter",key:"MESSAGE_TYPE_SPLIT_NODE",description:"",inputs:{in:{name:"In",messageTypes:AllMessageTypes}},outputs:Object.assign({},outputs),node({send:send,receive:receive,prop:prop}){receive((message=>{send(message,message.type)}))}};const TransposeNodeDef={name:"Transpose",key:"TRANSPOSE_NODE",description:"",inputs:{in:{name:"In",messageTypes:["noteon","noteoff"]}},outputs:{out:{name:"Out",messageTypes:["noteon","noteoff"]}},props:{transpose:{name:"Transpose",value:0}},node({send:send,receive:receive,prop:prop}){receive(((message,edge)=>{let transpose=prop("transpose"),transposed=clamp(message.data[1]+transpose,0,127),data=new Uint8Array([message.data[0],transposed,message.data[2]]);send(new MidiplexMessage(data),"out")}))}};const CCRangeNodeDef={name:"CC Range",key:"CC_RANGE_NODE",description:"Rescale CC messages to a smaller range. Useful for mapping a knob to a smaller range while preserving the control's full range of motion.",inputs:{in:{name:"In",messageTypes:["controlchange"]}},outputs:{out:{name:"Out",messageTypes:["controlchange"]}},props:{mapping:{name:"Range Mapping",value:{}}},node({prop:prop,send:send,receive:receive}){receive((message=>{let map=prop("mapping"),cc=message.data[1];if(void 0===map[cc]||null===map[cc])send(message,"out");else{let data=new Uint8Array([message.data[0],message.data[1],convertRange(message.data[2],0,127,map[cc][0],map[cc][1])]);send(new MidiplexMessage(data),"out")}}))}};const CCMapNodeDef={name:"CC Map",key:"CC_MAP_NODE",description:"Map control change messages to other control change destinations.",inputs:{in:{name:"In",messageTypes:["controlchange"]}},outputs:{out:{name:"Out",messageTypes:["controlchange"]}},props:{mapping:{name:"Mapping",value:{}}},node({prop:prop,send:send,receive:receive}){receive((message=>{let map=prop("mapping"),cc=message.data[1];if(map[cc]){let mData=message.data;map[cc].forEach((cc=>{send(new MidiplexMessage(new Uint8Array([mData[0],cc,mData[2]])),"out")}))}else send(message,"out")}))}};const TogglePathDef={name:"CC Toggle Path",key:"TOGGLE_PATH_NODE",description:"Toggles the output path open or closed via the given CC channel and value. The given CC value is always filtered out.",inputs:{in:{name:"In",messageTypes:AllMessageTypes}},outputs:{out:{name:"Out",messageTypes:AllMessageTypes}},props:{triggerOpen:{name:"Open Value",value:null},triggerClosed:{name:"Closed Value",value:null},open:{name:"Open",value:!0}},node({send:send,receive:receive,update:update,prop:prop}){let isOpen=prop("open");update((()=>{isOpen=prop("open")})),receive((message=>{if(isOpen){let triggerClosed=prop("triggerClosed");if(triggerClosed&&Util.Message.matchTrigger(triggerClosed,message))return void(isOpen=!1)}else{let triggerOpen=prop("triggerOpen");if(triggerOpen&&Util.Message.matchTrigger(triggerOpen,message))return void(isOpen=!0)}isOpen&&send(message,"out")}))}};const CustomFilterNodeDef={name:"Custom Filter",key:"CUSTOM_FILTER_NODE",description:"",inputs:{in:{name:"In",messageTypes:AllMessageTypes}},outputs:{out:{name:"Out",messageTypes:AllMessageTypes}},props:{filter:{name:"Filter Function",value:()=>!0}},node({send:send,receive:receive,prop:prop}){receive((message=>{prop("filter")(message)&&send(message,"out")}))}};const CCPassNodeDef={name:"CC Pass",key:"CC_PASS_NODE",description:"Specify which CC messages are allowed to pass through.",inputs:{in:{name:"In",messageTypes:["controlchange"]}},outputs:{out:{name:"Out",messageTypes:["controlchange"]}},props:{pass:{name:"Pass",value:[]}},node({prop:prop,send:send,receive:receive}){receive((message=>{prop("pass").includes(message.data[1])&&send(message,"out")}))}};const NoteMapNodeDef={name:"Note Map",key:"NOTE_MAP_NODE",description:"Map noteon/noteoff messages to other notes.",inputs:{in:{name:"In",messageTypes:["controlchange"]}},outputs:{out:{name:"Out",messageTypes:["controlchange"]}},props:{mapping:{name:"Mapping",value:{}}},node({prop:prop,send:send,receive:receive}){receive((message=>{let map=prop("mapping"),cc=message.data[1];if(map[cc]){let mData=message.data;map[cc].forEach((cc=>{send(new MidiplexMessage(new Uint8Array([mData[0],cc,mData[2]])),"out")}))}else send(message,"out")}))}};const SetChannelNodeDef={name:"Set Channel",key:"SET_CHANNEL_NODE",description:"Set a specific channel for all messages passing through this node. Non-channel messages are passed through unmodified.",inputs:{in:{name:"In",messageTypes:AllMessageTypes}},outputs:{out:{name:"Out",messageTypes:AllMessageTypes}},props:{channel:{name:"Channel",value:AllMessageTypes}},node({send:send,receive:receive,prop:prop}){receive((message=>{message.setChannel(prop("channel")),send(message,"out")}))}};const SplitChannelNodeDef={name:"Split Channel",key:"SPLIT_CHANNEL_NODE",description:'Split messages by channel. Each channel has its own output edge by number. Non-channel messages are passed through the edge "nonchannel"',inputs:{in:{name:"In",messageTypes:AllMessageTypes}},outputs:{nonchannel:{name:"Non-Channel",messageTypes:AllMessageTypes},1:{name:"Channel 1",messageTypes:ChannelMessageTypes},2:{name:"Channel 2",messageTypes:ChannelMessageTypes},3:{name:"Channel 3",messageTypes:ChannelMessageTypes},4:{name:"Channel 4",messageTypes:ChannelMessageTypes},5:{name:"Channel 5",messageTypes:ChannelMessageTypes},6:{name:"Channel 6",messageTypes:ChannelMessageTypes},7:{name:"Channel 7",messageTypes:ChannelMessageTypes},8:{name:"Channel 8",messageTypes:ChannelMessageTypes},9:{name:"Channel 9",messageTypes:ChannelMessageTypes},10:{name:"Channel 10",messageTypes:ChannelMessageTypes},11:{name:"Channel 11",messageTypes:ChannelMessageTypes},12:{name:"Channel 12",messageTypes:ChannelMessageTypes},13:{name:"Channel 13",messageTypes:ChannelMessageTypes},14:{name:"Channel 14",messageTypes:ChannelMessageTypes},15:{name:"Channel 15",messageTypes:ChannelMessageTypes},16:{name:"Channel 16",messageTypes:ChannelMessageTypes}},props:{},node({send:send,receive:receive,prop:prop}){receive((message=>{message.isChannelMessage?send(message,message.channel.toString()):send(message,"nonchannel")}))}};var nodes=Object.freeze({__proto__:null,CCMapNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,CCMapNodeDef,config)}},CCPassNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,CCPassNodeDef,config)}},CCRangeNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,CCRangeNodeDef,config)}},CustomFilterNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,CustomFilterNodeDef,config)}},DebugNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,DebugNodeDef,config)}},InputNode:class extends MidiplexNodeInstance{constructor(key){super(key,InputNodeDefinition)}},MessageTypePassNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,MessageTypePassNodeDef,config)}},MessageTypeSplitNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,MessageTypeSplitDef,config)}},NoteMapNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,NoteMapNodeDef,config)}},OutputNode:class extends MidiplexNodeInstance{constructor(key){super(key,OutputNodeDefinition)}},SetChannelNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,SetChannelNodeDef,config)}},SplitChannelNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,SplitChannelNodeDef,config)}},TogglePathNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,TogglePathDef,config)}},TransposeNode:class extends MidiplexNodeInstance{constructor(key,config={}){super(key,TransposeNodeDef,config)}}});exports.Nodes=nodes,exports.Util=Util,exports.WebMidi=wm}));
